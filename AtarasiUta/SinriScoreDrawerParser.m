...
-(NSInteger)parseNoteStringForNotation:(NSString*)c withFlag:(NSInteger)flag withNote:(NSMutableDictionary*)note{
...
-(NSArray*)parseNoteStringForNotationAddition:(NSMutableDictionary*)note{
    NSLog(@"debug: %s, %d within %s",__FILE__,__LINE__,__FUNCTION__);
    if([note objectForKey:@"_times_multiply"] && [[note objectForKey:@"_times_multiply"] integerValue]>0){
        NSInteger x=[[note objectForKey:@"_times_multiply"] integerValue];
        [note setObject:[NSNumber numberWithInteger:(x-1)] forKey:@"_has_long_line"];
    }
    if([note objectForKey:@"_times_divided"] && [[note objectForKey:@"_times_divided"] integerValue]>0){
        if([note objectForKey:@"_times_divided"] && [[note objectForKey:@"_times_divided"]integerValue]==3){
            [note setObject:@YES forKey:@"triplets"];
        }
        else if ([note objectForKey:@"_times_divided"] && [[note objectForKey:@"_times_divided"]integerValue]%2==0){
            NSInteger x=[[note objectForKey:@"_times_divided"]integerValue];
            [note setObject:[NSNumber numberWithInteger:(x/2)] forKey:@"underlines"];
        }
        else{
            NSInteger x=[[note objectForKey:@"_times_divided"]integerValue];
            [note setObject:[NSNumber numberWithInteger:(x/2)] forKey:@"underlines"];
    ...
    NSInteger has_long_line=[note objectForKey: @"_has_long_line"]?[[note objectForKey: @"_has_long_line"] integerValue]:0;
    Boolean has_dot=[note objectForKey:@"dot"]?[[note objectForKey:@"dot"] boolValue]:NO;
    
    [note removeObjectForKey:@"_has_long_line"];
    [note removeObjectForKey:@"_times_divided"];
    [note removeObjectForKey:@"_times_multiply"];
    [note removeObjectForKey:@"dot"];
    
    NSMutableArray* notes=[@[note] mutableCopy];
    if(has_dot){
        [notes addObject:@{@"special_note":@"DOT"}];
    }
    for(int j=0;j<has_long_line;j++){
        [notes addObject:@{@"special_note":@"LONGER_LINE"}];
    }
